...
// Cost for not having [0..6] roads within 7.5m:
OPT_fnc_distance_to_roads = [true, 0, 6,
			     '[_x, 7.5]',
			     fnc_roads_nearby]
	                     call fnc_to_cost_function;
...
// Cost function for being too far from designated targets:
OPT_fnc_distance_to_targets = [false, 75, 150,
  			       '[position _x,
                                 ([[["_t"], {position _t}] call fnc_lambda,
                                   _x getVariable "targets"] 
                                   call fnc_map) call fnc_vector_mean]',
...
// Cost function for being too far from designated targets:
OPT_fnc_long_distance_to_targets = [false, 150, 300,
				    '[position _x,
				      ([[["_t"], {position _t}] call fnc_lambda,
					_x getVariable "targets"] 
					call fnc_map) call fnc_vector_mean]',
